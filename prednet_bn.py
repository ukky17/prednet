...
class PredNet(Recurrent):
    ...
    def build(self, input_shape):
        ...
        self.upsample = []
        self.pool = []
        self.bn_layers = {'a': [], 'ahat': []}
        self.act_layers = {'a': [], 'ahat': []}
        ...
        for l in range(self.nb_layers):
            ...
            if self.data_format == 'channels_last':
                ...
                bn_axis = 1
            self.bn_layers['ahat'].append(BatchNormalization(axis=bn_axis))
            self.act_layers['ahat'].append(Activation('relu'))
            ...
            if l < self.nb_layers - 1:
                ...
                if self.data_format == 'channels_last':
                    ...
                    bn_axis = 1
                self.bn_layers['a'].append(BatchNormalization(axis=bn_axis))
                self.act_layers['a'].append(Activation('relu'))
        ...
        for c in sorted(self.conv_layers.keys()):
            ...
            for l in range(len(self.conv_layers[c])):
                ...
                if c == 'a':
                    out_shape = (in_shape[0], self.stack_sizes[l + 1], in_shape[2] // self.A_stride_sizes[l], in_shape[3] // self.A_stride_sizes[l])
                elif c == 'ahat':
                    out_shape = (in_shape[0], in_shape[1], in_shape[2] // self.Ahat_stride_sizes[l], in_shape[3] // self.Ahat_stride_sizes[l])
                if l < self.nb_layers - 1:
                    ds_factor *= self.upsample_size[l]
                if self.data_format == 'channels_last':
                    in_shape = (in_shape[0], in_shape[2], in_shape[3], in_shape[1])
                    out_shape = (out_shape[0], out_shape[2], out_shape[3], out_shape[1])
                    # print(c, l, in_shape, out_shape)
                with K.name_scope('layer_' + c + '_' + str(l)):
                    self.conv_layers[c][l].build(in_shape)
                    if c in ['a', 'ahat']:
                        self.bn_layers[c][l].build(out_shape)
                self.trainable_weights += self.conv_layers[c][l].trainable_weights
                if c in ['a', 'ahat']:
                    self.trainable_weights += self.bn_layers[c][l].trainable_weights
    ...
    def step(self, a, states):
        ...
        for l in range(self.nb_layers):
            ...
            ahat = self.bn_layers['ahat'][l].call(ahat)
            ahat = self.act_layers['ahat'][l].call(ahat)
            if l == 0:
            ...
            if l < self.nb_layers - 1:
                ...
                a = self.bn_layers['a'][l].call(a)
                a = self.act_layers['a'][l].call(a)
